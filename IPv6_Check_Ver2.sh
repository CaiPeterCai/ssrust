...
ping_ipv6() {
    ...
    ip addr del $src_ip/112 dev $interface_name  # 改为 /112
...
cleanup() {
    for src_ip in "${ip_array[@]}"; do
        sudo ip addr del "$src_ip"/112 dev $interface_name 2>/dev/null  # 改为 /112
    done
...
# 修改前缀计算，使用前7个部分而不是4个部分（对于/112前缀）
current_prefix=$(echo $current_ipv6 | awk -F':' '{print $1":"$2":"$3":"$4":"$5":"$6":"$7}')
echo ""
echo "网卡当前配置的IPv6： $current_ipv6"
echo "分配该虚拟机的IPv6： $current_prefix::/112"  # 改为 /112
echo ""
...
declare -a ip_array
echo ""
echo "在 $current_prefix::/112 中生成$ipv6_num个IPv6进行检测 请等待任务完成"  # 改为 /112
...
for (( i=0; i<$ipv6_num; i++ )); do
    while : ; do
        # 只生成最后16位的随机部分（对于/112前缀）
        random_part=$(printf '%x' $((RANDOM%65536)))
        test_ipv6="$current_prefix:$random_part"
        if [ -z ${used_ip_addrs[$test_ipv6]} ]; then
            sudo ip addr add "$test_ipv6"/112 dev $interface_name 2>/dev/null  # 改为 /112
